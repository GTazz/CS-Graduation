...
print_title("Carneirinhos")

sheep = 0
awake = "S0"
while True:
    while awake not in "SN":
        awake = input("Tá acordado ainda? [S/N] ").strip()[0].upper()
    if awake == "S":
        awake = "S0"
        sheep += 1
        print(f"{sheep} carneirinho\n")
    else:
        break
        
print(f"\nO programador dormiu após contar {sheep} carneirinhos.\n")
